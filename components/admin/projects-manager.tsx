...
import { ArrowLeft, ExternalLink, Github, Loader2, Pencil, Plus, Save, Star, Trash2, Upload, X } from "lucide-react"
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <section>
          <form id="projects-form" onSubmit={handleSubmit} className="bg-card border border-border rounded-lg p-6 space-y-5 sticky top-24">
            ...
            <div>
              <label className="block text-sm font-mono text-muted-foreground mb-2">image</label>
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
              ...
              {imageUrl ? (
                <div className="relative rounded-lg overflow-hidden border border-border">
                  <img src={imageUrl} alt="Project preview" className="w-full h-40 object-cover" />
                  <button
                    type="button"
                    onClick={() => setImageUrl("")}
                    className="absolute top-2 right-2 p-1.5 rounded-full bg-background/80 text-destructive hover:bg-background transition-all"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </div>
                ...
                <button
                  ...
                  disabled={uploading}
                  className="w-full h-24 border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center gap-2 text-muted-foreground hover:border-primary hover:text-foreground transition-all disabled:opacity-50"
                >
                  {uploading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Upload className="h-5 w-5" />}
                  <span className="text-xs font-mono">{uploading ? "Uploading…" : "Click to upload image"}</span>
                </button>
              )}
            </div>
